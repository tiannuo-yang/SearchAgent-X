...
public:
    ...
    py::object early_stop(float step_low_ema)
    {
        std::vector<size_t> stop_req_ids = appr_alg->early_stop(step_low_ema);
        size_t *req_ids = new size_t[stop_req_ids.size()];
        std::copy(stop_req_ids.begin(), stop_req_ids.end(), req_ids);
        py::capsule free_when_done(req_ids, [](void *f)
                                   { delete[] f; });
        return py::array_t<size_t>(
            {stop_req_ids.size()}, // shape
            {sizeof(size_t)},      // C-style contiguous strides for each index
            req_ids,               // the data pointer
            free_when_done);
    }

    // 现在的bf_search是单query内多线程并行，不支持batch query并行
    py::object bfKnnQuery_return_numpy(
        py::object input,
        size_t k = 1,
        int num_threads = -1,
        const std::function<bool(hnswlib::labeltype)> &filter = nullptr)
    {
...

        {
            ...
            if (normalize == false)
            {
                std::priority_queue<std::pair<dist_t, hnswlib::labeltype>> result = appr_alg->bfSearchKnn(
                    (void *)items.data(0), k, p_idFilter, num_threads);
                if (result.size() != k)
                {
                    std::cerr << result.size() << " " << k << std::endl;
            ...
            else
            {
                ...
                float *data = (float *)items.data(0);
                normalize_vector((float *)items.data(0), (norm_array.data()));

                std::priority_queue<std::pair<dist_t, hnswlib::labeltype>> result = appr_alg->bfSearchKnn(
                    (void *)(norm_array.data()), k, p_idFilter, num_threads);
                if (result.size() != k)
                    throw std::runtime_error(
                        "Cannot return the results in a contiguous 2D array. Probably ef or M is too small");
                for (int i = k - 1; i >= 0; i--)
    ...
    py::object knnQuery_return_numpy_with_stall(
        py::object input,
        size_t k = 1,
        size_t req_id = -1,
        const std::function<bool(hnswlib::labeltype)> &filter = nullptr)
    {
...

        {
            ...
            // efficient rag use single query in one request
            if (rows == 1)
            {
                // Create new thread for truely using multi-core
                std::priority_queue<std::pair<dist_t, hnswlib::labeltype>> result;
                std::thread searchThread(
                    [&]()
                    {
                        result = appr_alg->searchKnnWithStall((void*)items.data(0), k, p_idFilter, &is_stop_by_stall, req_id);
                ...
                searchThread.join();
                // std::priority_queue<std::pair<dist_t, hnswlib::labeltype >> result = appr_alg->searchKnnWithStall(
                //     (void*)items.data(0), k, p_idFilter, &is_stop_by_stall, req_id);
            }
            else
            {
                throw std::runtime_error("should not be here, knnsearch_with_stall can't be rows > 1");
            }
        }
        py::capsule free_when_done_l(data_numpy_l, [](void *f)
                                     { delete[] f; });
...
PYBIND11_PLUGIN(hnswlib)
{
    ...
    py::class_<Index<float>>(m, "Index")
        ...
        .def("bf_knn_query",
             &Index<float>::bfKnnQuery_return_numpy,
             py::arg("data"),
             py::arg("k") = 1,
             py::arg("num_threads") = -1,
             py::arg("filter") = py::none())
        .def("early_stop",
             &Index<float>::early_stop,
             py::arg("step_low_ema"))
        .def("knn_query",
             ...
             py::arg("num_threads") = -1,
             py::arg("filter") = py::none())
        .def("knn_query_with_stall",
             &Index<float>::knnQuery_return_numpy_with_stall,
             py::arg("data"),
             py::arg("k") = 1,
             py::arg("req_id") = -1,
             py::arg("filter") = py::none())
        ...
        .def(py::pickle([](const Index<float> &ind) {    // __getstate__
