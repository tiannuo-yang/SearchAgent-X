    ...
    def testIndexBf(self):
        ...
        def early_stop():
            for i in range(num_queries*2):
                index.early_stop(step_low_ema=0.01) # ☆☆☆
                time.sleep(0.01)
        t = threading.Thread(target=early_stop)
        t.start()

        start = time.time_ns()
        for i in range(num_queries):
            query = querys[i]
            labels, dists, is_stop_by_stall = index.knn_query_with_stall(query, k=k, req_id=i) # ☆☆☆ one more return value
            labels2.append(labels[0])
            is_stop.append(is_stop_by_stall)
        end = time.time_ns()
        # 这里平均返回时间超出early_stop_time_us一些是因为后面有些query没有受到early stop
        # print(f"Early Stop {early_stop_time_us}us Time each taken for {num_queries} queries: {(end - start)/(num_queries*1000*1000)} ms")
