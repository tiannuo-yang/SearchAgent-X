...
class BlockSpaceManagerV1(BlockSpaceManager):
    ...
    def can_allocate(self, seq_group: SequenceGroup) -> AllocStatus:
        ...
        if (self.num_total_gpu_blocks - num_required_blocks <
            ...
            return AllocStatus.NEVER, None
        if num_free_gpu_blocks - num_required_blocks >= self.watermark_blocks:
            return AllocStatus.OK, self.probe_prefix_hit_count(seq_group) if self.enable_caching else None
        else:
            return AllocStatus.LATER, None
        
    def probe_prefix_hit_count(self, seq_group: SequenceGroup) -> float:
        seq = seq_group.get_seqs(status=SequenceStatus.WAITING)[0]
        num_prompt_blocks = len(seq.logical_token_blocks)
        hit_count = 0
        for logical_idx in range(num_prompt_blocks):
            if seq.hash_of_block(logical_idx) in self.gpu_allocator.evictor:
                hit_count += 1
        return hit_count
    ...
    def allocate(self, seq_group: SequenceGroup) -> None:
        ...
        for logical_idx in range(num_prompt_blocks):
            if (self.block_sliding_window is not None
                ...
                block, hit_flag = self.gpu_allocator.allocate(
                    seq.hash_of_block(logical_idx),
                    seq.num_hashed_tokens_of_block(logical_idx))
                if hit_flag:
                    hit_count += 1
            else:
                block = self.gpu_allocator.allocate()[0]
                # Set the reference counts of the token blocks.
                block.ref_count = seq_group.num_seqs()
    ...
    def _promote_last_block(
        ...
        if self.gpu_allocator.contains_block(new_hash):
            ...
            return self.gpu_allocator.allocate(new_hash)[0]
        else:
    ...
    def _allocate_last_physical_block(
        ...
        if not self.enable_caching:
            return self.gpu_allocator.allocate()[0]
        block_hash: Optional[int] = None
        if (self._is_last_block_full(seq)):
        ...
        # prefix tokens)
        new_block = self.gpu_allocator.allocate(block_hash, num_hashed_tokens)[0]
            ...
            for cpu_block in block_table:
                if cpu_block in mapping:
                    ...
                    gpu_block = self.gpu_allocator.allocate(
                        cpu_block.block_hash, cpu_block.num_hashed_tokens)[0]
            ...
            for gpu_block in block_table:
                if gpu_block in mapping:
                    ...
                    cpu_block = self.cpu_allocator.allocate(
                        gpu_block.block_hash, gpu_block.num_hashed_tokens)[0]
